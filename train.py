...
# optimizer
optimizer = optim.Adam(bi_encoder.passage_encoder.parameters(), lr=1e-5)  # optimize only BERT
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        for batch in val_loader:
            descs, seqs = batch
            descs = [desc.to(device) for desc in descs]
            protein_sequences = [(f"protein_{i}", seq) for i, seq in enumerate(seqs)]
            protein_embeddings, description_embeddings = bi_encoder(protein_sequences, list(descs))
